...
import {
  ...
  Edit, Plus, Printer, Wrench, X, Search, FileText, DollarSign, Calendar, Download
} from "lucide-react";
import { ImportarPedidosDialog } from "../components/importar-pedidos-dialog";
...
export function PedidosListaClient({
  pedidosIniciais,
  dataInicialInicial,
  dataFinalInicial,
  vendedor,
  ...
  dataInicialInicial: string;
  dataFinalInicial: string;
  vendedor: number;
}) {
  const [dados, setDados] = useState<pedido[]>(pedidosIniciais);
  const [dadosFiltro, setDadosFiltro] = useState<pedido[]>(pedidosIniciais);
  const [pesquisa, setPesquisa] = useState<string>('');
  const [filtroSituacao, setFiltroSituacao] = useState<string>('full');
  const [dataInicial, setDataInicial] = useState<string>(dataInicialInicial);
  const [dataFinal, setDataFinal] = useState<string>(dataFinalInicial);
  ...
  function handleOrder(codigo: number) {
    router.push(`/marketplaces/pedidos/${codigo}`);
  }
  ...
  return (
    <div className="min-h-screen flex flex-col sm:ml-52 p-2 sm:p-4 lg:p-6 w-full h-full justify-itens-center items-start bg-slate-100">
              ...
              <Table className="w-full">
                <TableHeader className="bg-gray-50">
                  <TableRow>
                    ...
                    <TableHead className="text-gray-600 font-medium text-center w-20 sm:w-32 text-xs sm:text-sm">Plataforma</TableHead>
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="text-gray-700 text-xs sm:text-sm py-2 sm:py-3">
                          <span className="block truncate max-w-[120px] sm:max-w-none">{item.cliente?.nome}</span>
                        </TableCell>
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      router.push(`/marketplaces/pedidos/${item.codigo}/imprimir`);
                                    }}
                                  >
      ...
      <ImportarPedidosDialog
        ...
        onSuccess={() => buscar(dataInicial, dataFinal, pesquisa)}
      />
    </div>
  )
}