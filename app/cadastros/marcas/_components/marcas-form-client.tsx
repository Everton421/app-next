...
export function MarcasFormClient({
    dadosIniciais,
    idInicial,
}: {
    dadosIniciais: marca | null;
    idInicial?: string;
}) {
    const api = configApi();
    ...
    async function gravar() {
        ...
        try {
            const result = isEdit
                ? await api.put("/next/marcas", data, { headers: { token: user.token } })
                : await api.post("/offline/marcas", data, { headers: { token: user.token } });

            if (result.status === 200 && result.data?.codigo > 0) {
                setVisibleAlert(true);
                setMsgAlert(`Marca ${data?.descricao} ${isEdit ? "alterada" : "cadastrada"} com Sucesso!`);
            }
        } catch (e: any) {
            console.error("Erro ao gravar marca:", e);
            setMsgAlert(`Erro ao ${isEdit ? "salvar alterações da" : "gravar"} Marca ${data?.codigo ?? ""}.`);
            setVisibleAlert(true);
        } finally {
            setIsSaving(false);
        }
        ...
        <div className="min-h-screen flex flex-col sm:ml-56 p-4 bg-slate-100 space-y-6 pb-20">
            ...
            <div className="w-full max-w-screen-2xl mx-auto bg-white rounded-lg shadow-md p-4 md:p-6 lg:p-8 flex flex-col flex-1">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <Button variant="outline" onClick={() => router.push("/cadastros/marcas")}>
