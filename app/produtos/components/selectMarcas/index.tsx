...
export default function SelectMarca({ codigoMarca, setMarca}:props ){
    ...
    ()=>{
        async function buscaMarcas(){
            if(pesquisa !== ''){
             try{
                    setLoading(true)
                    const result = await api.get(`/marcas`,{
                        headers:{ 
                            token:  user.token
                        },
                        params:{
                ...
                dadosMarca.length > 0 && (
                    <div className="absolute z-10    ">
                        { dadosMarca.slice(0, 2).map((i: any) => (
                            <div key={i.codigo} onClick={() => seleciona(i)} className="   sm:ml-56 p-2 m-1 shadow-lg bg-gray-500 border-current rounded-md cursor-pointer  ">
